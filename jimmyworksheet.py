...
def get_app_layout(username):
    ...
    return html.Div([
        ...
        html.Div([
            html.Label("單點預算限制: ", style={"fontSize": "14px"}),
            dcc.Input(
                id="budget", type="number", placeholder="單點篩選預算", value=None, style={"width": "120px", "padding": "6px"}
            ),
        ], style={"display": "inline-block", "marginLeft": "10px"}),

    # 2. 規劃基準：整趟總預算 (新增 ID: total-trip-budget)
        html.Div([
            html.Label("整趟行程總預算: ", style={"fontSize": "14px", "fontWeight": "bold"}),
            dcc.Input(
                id="total-trip-budget", type="number", placeholder="總金額", value=None, style=STYLES["input_budget"]
            ),
        ], style={"display": "inline-block", "marginLeft": "10px"}),
        dcc.Dropdown(
                id="category",
                ...
                style={"width": "200px", "verticalAlign": "middle", "display": "inline-block"}
            ),
